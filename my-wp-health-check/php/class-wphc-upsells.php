...
class WPHC_Upsells {
	...
	public static function three_week_upsell_notice() {
		if ( self::should_show_three_week_upsell() ) {
			...
			$screen_id    = $screen->id;
			if ( 'plugins.php' == $screenparent && 'plugins' == $screen_id && current_user_can( 'install_plugins' ) ) {
				...
				// Add a nonce to the dismissal URL.
				$nope_url = esc_url(
					add_query_arg(
						array(
							'wphc_three_week_upsell_notice_check' => 'remove_message',
							'_wpnonce' => wp_create_nonce( 'wphc_upsell_notice_nonce' ),
						),
						admin_url()
					)
				);
				...
				<div class="updated wphc-upsell-message">
					...
					<?php
					esc_html_e(
						'Hey there! Sorry to bother you. I noticed that you have been using SiteAlert for three weeks now. SiteAlert Premium has 
                              even more enhanced monitoring for your site including:',
						'my-wp-health-check'
					);
					?>
						</p>
	...
	public static function check_upsell_notice_button_clicks() {
		if ( isset( $_GET['wphc_three_week_upsell_notice_check'], $_GET['_wpnonce'] ) &&
			wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'wphc_upsell_notice_nonce' ) &&
			'remove_message' === sanitize_text_field( wp_unslash( $_GET['wphc_three_week_upsell_notice_check'] ) ) ) {

			update_option( 'wphc_three_week_upsell_notice_dismissed', true );
		}
	}
