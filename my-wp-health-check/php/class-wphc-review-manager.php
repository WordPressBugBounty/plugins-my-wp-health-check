<?php
/**
 * Handles displaying of review notice.
 *
...
class WPHC_Review_Manager {
	...
	public function display_admin_message() {
		// Generate nonce.
		$nonce = wp_create_nonce( 'wphc_review_notice_nonce' );

		// Add nonce to URLs.
		$already_url = esc_url(
			add_query_arg(
				array(
					'wphc_review_notice_check' => 'already_did',
					'_wpnonce'                 => $nonce,
				)
			)
		);
		...
		$nope_url = esc_url(
			add_query_arg(
				array(
					'wphc_review_notice_check' => 'remove_message',
					'_wpnonce'                 => $nonce,
				)
			)
		);
		...
		<div class="updated wphc-review-message">
			<p>
				<?php
				printf(
					// Translators: %s is the signature of the SiteAlert team.
					esc_html__(
						'Greetings! We just noticed that you have been using the SiteAlert plugin for over a week now. That is
		awesome! Could you please help us out by giving this plugin a 5-star rating on WordPress? This
		will help us by helping other users discover this plugin. And if you want to try our premium features, we would be happy to send you a coupon code. Message us through https://sitealert.io/contact/ %s',
						'my-wp-health-check'
					),
					wp_kses_post( '<strong><em>~ The SiteAlert Team</em></strong>' )
				);
				?>
			</p>
	...
	public function admin_notice_check() {
		...
		if ( isset( $_GET['wphc_review_notice_check'], $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'wphc_review_notice_nonce' ) ) {
			if ( 'remove_message' === $_GET['wphc_review_notice_check'] ) {
				update_option( 'wphc_review_message_trigger', -1 );
			}

			if ( 'already_did' === $_GET['wphc_review_notice_check'] ) {
				update_option( 'wphc_review_message_trigger', -1 );
			}
		}
	}
