...
class WPHC_Checks {
	...
	public function wordpress_version_check() {
		...
		// Prepares messages.
		$good  = esc_html__( 'Your WordPress is up to date. Great job!', 'my-wp-health-check' );
		$error = esc_html__( 'Encountered an error. WordPress version not checked. Please check again later.', 'my-wp-health-check' );
		$bad   = esc_html__( 'Your WordPress is not up to date. Your site has not received the latest security fixes and is less secure from hackers. Please consider updating.', 'my-wp-health-check' );
	...
	/**
	 * Checks if using the latest version of plugins
	 *
	 * @since 0.2.0
	 * @return array The array of the message and type
	 */
	public function update_plugins_check() {
		...
		$learn_more = sprintf(
			...
			esc_url( 'https://sitealert.io/keeping-your-wordpress-site-updated/?utm_campaign=health-plugin&utm_medium=plugin&utm_source=checks-page&utm_content=updated-plugin-check' ),
			esc_html__( 'Learn more about keeping your site updated.', 'my-wp-health-check' )
		);

		// Ensure plugin update functions are available.
		...
		if ( ! empty( $plugin_updates ) ) {
			...
			foreach ( $plugin_updates as $plugin ) {
				// Use snake_case variable to comply with WordPress standards.
				$plugin_name = $plugin->Name;
				$plugins[] = esc_html( $plugin_name );
			}
			$plugin_list = implode( ', ', $plugins );

			$message = sprintf(
				// Translators: %s is the list of plugins with available updates.
				esc_html__( 'You are not using the latest version of these plugins: %s. These updates could contain important security updates. Please update your plugins to ensure your site is secure and safe.', 'my-wp-health-check' ),
				$plugin_list
			);

			return $this->prepare_array( $message . ' ' . $learn_more, 'bad', 'plugin_updates', $plugins );
		} else {
			// Return a success message if all plugins are up to date.
			$message = esc_html__( 'All of your WordPress plugins are up to date. Great job!', 'my-wp-health-check' );
			return $this->prepare_array( $message . ' ' . $learn_more, 'good', 'plugin_updates', array() );
		}
	}
	...
	/**
	 * Checks if using the latest version of MySQL
	 *
	 * @since 0.1.0
	 */
	public function mysql_check() {
		...
		$version = wp_cache_get( 'wphc_mysql_version' );

		// If not cached, retrieve the version from the database and cache it.
		if ( false === $version ) {
			if ( method_exists( $wpdb, 'db_version' ) ) {
				$mysql_server_type = $wpdb->db_version();

				// Secure the query using a prepared statement.
				$version = $wpdb->get_var( 'SELECT VERSION()' ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery

				// Cache the result for 24 hours.
				wp_cache_set( 'wphc_mysql_version', esc_html( $version ), '', DAY_IN_SECONDS );
			}
		}
		...
		$error               = esc_html__( 'Error checking database health.', 'my-wp-health-check' );
		$values              = array(
			'app'     => 'MySQL',
			'version' => esc_html( $version ),
		);

		// MySQL versions and release dates.
		...
		// MariaDB versions and release dates.
		...
		if ( stripos( $mysql_server_type, 'mariadb' ) !== false ) {
			$mariadb       = true;
			$values['app'] = 'MariaDB';
		}
		...
		$db_version        = esc_html( $version_array[0] . '.' . $version_array[1] );
		...
		if ( $eol_time <= $today ) {
			$msg = sprintf(
				// Translators: %1$s is the database type (MySQL or MariaDB), %2$s is the version number, and %3$s is the end-of-life date.
				esc_html__( 'Your server is running %1$s version %2$s which has not been supported since %3$s.', 'my-wp-health-check' ),
				esc_html( $values['app'] ),
				esc_html( $db_version ),
				esc_html( $mariadb ? $mariadb_versions[ $db_version ]['eol'] : $mysql_versions[ $db_version ]['eol'] )
			);
		} else {
			$msg = sprintf(
				// Translators: %1$s is the database type (MySQL or MariaDB), %2$s is the version number, and %3$s is the date until which security updates will be provided.
				esc_html__( 'Good job! Your server is running %1$s version %2$s which will receive security updates until %3$s.', 'my-wp-health-check' ),
				esc_html( $values['app'] ),
				esc_html( $db_version ),
				esc_html( $mariadb ? $mariadb_versions[ $db_version ]['eol'] : $mysql_versions[ $db_version ]['eol'] )
			);
		...
		return $this->prepare_array( $msg, 'good', 'db_version', $values );
	}
