...
class WPHC_Telemetry {
	...
	private function send_data() {
		$response = wp_remote_post(
			'https://telemetry.sitealert.io/api/v1/telemetry',
			array(
				'method'      => 'POST',
				'timeout'     => 15,
				'redirection' => 5,
	...
	public function admin_notice() {
		...
		if ( false !== stristr( network_site_url( '/' ), 'dev' ) || false !== stristr( network_site_url( '/' ), 'localhost' ) || false !== stristr( network_site_url( '/' ), ':8888' ) ) {
			...
			// Generate nonce.
			$nonce = wp_create_nonce( 'wphc_admin_notice_nonce' );

			// Add nonce to URLs.
			$optin_url  = esc_url(
				add_query_arg(
					array(
						'wphc_track_check' => 'opt_into_tracking',
						'_wpnonce'         => $nonce,
					)
				)
			);
			$optout_url = esc_url(
				add_query_arg(
					array(
						'wphc_track_check' => 'opt_out_of_tracking',
						'_wpnonce'         => $nonce,
					)
				)
			);
	...
	public function admin_notice_check() {
		if ( isset( $_GET['wphc_track_check'], $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'wphc_admin_notice_nonce' ) ) {
			$settings = (array) get_option( 'wphc-settings' );
